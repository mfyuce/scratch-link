...
namespace scratch_lang
{
    ...
    public static class CodeConversion
    {
        ...
        public static Dictionary<string, string> Arduino { get; set; } =
            ...
            {"control_if", "\n{indentation}if({child0}){\n{next_indentation}{child1}\n{indentation}}" },
            {"operator_equals", "(({child0})==({child1}))" },
            {"operator_random",  "random((convertToInt({child0})),(convertToInt({child1})))" },
            {"operator_subtract", "((convertToInt({child0}))-(convertToInt({child1})))" },
            {"operator_add", "(({child0})+({child1}))" },
            {"operator_join",  "(({child0})+({child1}))" },
            {"operator_contains", "((convertToStr({child0})).indexOf(convertToStr({child1})) > 0)" },
            {"operator_letter_of", "((convertToStr({child0})).charAt(convertToInt({child1})))" },
            {"operator_length",  "((convertToStr({child0})).length())" },
            {"operator_multiply",  "((convertToInt({child0}))*(convertToInt({child1})))" },
            {"operator_divide",  "((convertToInt({child0}))/(convertToInt({child1})))" },
            {"operator_or",  "(({child0})|({child1}))" },
            {"operator_gt", "(({child0})>({child1}))" },
            {"operator_lt", "(({child0})<({child1}))" },
            {"operator_and",  "(({child0})&&({child1}))" },
            {"operator_mod",  "(({child0})%({child1}))" },
            {"operator_round", "(round({child0}))" },
            {"operator_mathop",  "({child0}(convertToInt({child1})))" },
            {"operator_not",  "!({child0})" },
            {"data_variable",  "({child0})" },
            {"data_variable_str",  "({child0})" },
            {"text", "(\"{child0}\")" },
